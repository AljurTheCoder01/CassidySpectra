...
const newUserData: UserData = { ...userData };
    ...
    if (subcommand === "fight") {
      ...
      if (winChance > 0.5) {
        ...
        newUserData.adventure.health = Math.max(0, newUserData.adventure.health - 10);
        newTargetUserData.adventure.health = Math.max(0, newTargetUserData.adventure.health - 20);
        resultMessage = [
          `✅ **${userData.adventure.name} won the fight against ${targetUserData.adventure.name}!** ${UNISpectra.charm}`,
          `${UNISpectra.standardLine}`,
          `𝗘𝗮𝗿𝗻𝗲𝗱: ${coinsWon} 𝖼𝗈𝗂𝗇𝗌 💵`,
          `𝗬𝗼𝘂𝗿 𝗛𝗲𝗮𝗹𝘁𝗵: ${newUserData.adventure.health}/𝟣𝟢𝟢`,
          `${targetUserData.adventure.name}'𝘀 𝗛𝗲𝗮𝗹𝘁𝗵: ${newTargetUserData.adventure.health}/𝟣𝟢𝟢`,
          `━━━━━━━ ✕ ━━━━━━`
        ].join("\n");
      } else {
        const coinsLost = Math.floor(Math.random() * 51) + 10;
        newUserData.money = Math.max(0, (newUserData.money || 0) - coinsLost);
        newTargetUserData.money = (newTargetUserData.money || 0) + coinsLost;
        newUserData.adventure.health = Math.max(0, newUserData.adventure.health - 20);
        newTargetUserData.adventure.health = Math.max(0, newTargetUserData.adventure.health - 10);
        resultMessage = [
          `❌ **${targetUserData.adventure.name} won the fight against ${userData.adventure.name}!** ${UNISpectra.charm}`,
          `${UNISpectra.standardLine}`,
          `𝗟𝗼𝘀𝘁: ${coinsLost} 𝖼𝗈𝗂𝗇𝗌 💵`,
          `𝗬𝗼𝘂𝗿 𝗛𝗲𝗮𝗹𝘁𝗵: ${newUserData.adventure.health}/𝟣𝟢𝟢`,
          `${targetUserData.adventure.name}'𝘀 𝗛𝗲𝗮𝗹𝘁𝗵: ${newTargetUserData.adventure.health}/𝟣𝟢𝟢`,
          `━━━━━━━ ✕ ━━━━━━`
        ].join("\n");
    ...
    if (subcommand === "rest") {
      ...
      const newUserData: UserData = { ...userData };
      const healthRecovered = Math.floor(Math.random() * 20) + 10;
